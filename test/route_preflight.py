...
def verify_endpoint_mapping_contract(
    ...
    coordinate_case_end = action.find(
        "caseWORK_PHASE_ENDPOINT_WAIT_LOCAL_BASE:",
        coordinate_case_index,
    )
    endpoint_coordinate_case = action[
        coordinate_case_index:coordinate_case_end
    ]
    ring_one_branch_index = endpoint_coordinate_case.find(
        "if(ring==1U){"
    )
    ring_three_handoff_index = endpoint_coordinate_case.find(
        "workActionPhase="
        "WORK_PHASE_ENDPOINT_WAIT_RING3_RAISE;",
        ring_one_branch_index,
    )
    ring_one_break_index = endpoint_coordinate_case.find(
        "break;", ring_three_handoff_index
    )
    map_index = endpoint_coordinate_case.find(
        "completeEndpointMapAndStartTransfers();",
        ring_one_break_index,
    )
    ring_three_wait_index = action.find(
        "caseWORK_PHASE_ENDPOINT_WAIT_RING3_RAISE:"
    )
    ring_three_wait_end = action.find(
        "caseWORK_PHASE_CIRCLE_WAIT_ARM_LOWER:",
        ring_three_wait_index,
    )
    ring_three_wait = action[
        ring_three_wait_index:ring_three_wait_end
    ]
    third_scan_index = ring_three_wait.find(
        "beginDirectRing3EndpointScan();"
    )
    require(
        first_scan_index >= 0
        and 0
        <= ring_one_branch_index
        < ring_three_handoff_index
        < ring_one_break_index
        < map_index
        and ring_three_wait_index >= 0
        and third_scan_index >= 0,
