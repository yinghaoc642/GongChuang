...
// 搬运时M6已回安全零点后才允许M5大角度转动；使用仍低于旧版35000的
// 中等加速度，减少六次搬运的固定耗时，但端点视觉继续使用上面的柔和曲线。
const float ARM_BASE_TRANSFER_MAXIMUM_STEP_RATE = 52000.0f;
const float ARM_BASE_TRANSFER_STEP_ACCELERATION = 24000.0f;
...
constexpr uint16_t M6_SPEED_RPM = 260U;
constexpr uint8_t M6_ACCELERATION = 128U;
// 端点搜索/闭环是短距离、空载精密动作，使用更柔和的独立曲线。
constexpr uint16_t ENDPOINT_M6_SPEED_RPM = 160U;
constexpr uint8_t ENDPOINT_M6_ACCELERATION = 96U;
...
constexpr uint8_t M7_ACCELERATION = 220U;
/*
...
constexpr float ARM_AXIS_TERMINAL_VERIFY_TOLERANCE_MM = 0.20f;
 ...
 * 横移到3号的-20°视野。粗观察压到6 px内后下降15 mm，最终连续两个
...
constexpr float ENDPOINT_RING3_SEARCH_MINIMUM_ANGLE_DEGREES = -30.0f;
constexpr float ENDPOINT_RING3_SEARCH_MAXIMUM_ANGLE_DEGREES = -10.0f;
...
constexpr uint32_t ENDPOINT_RING_SCAN_TIMEOUT_MS = 15000UL;
constexpr uint32_t ENDPOINT_MAP_TOTAL_TIMEOUT_MS = 27000UL;
...
void serviceLinearAxisRecovery(LinearAxisMotion &axis) {
  ...
  /*
   * M5由需要高频serviceM5()的STEP/DIR状态机驱动。即使上层以后误把两轴
   * 同时启动，M6也必须等M5完全停稳后才读位置和降档重试，避免恢复代码
   * 的串口等待饿死M5脉冲服务。
   */
  if (axis.address == extensionAxis.address &&
...
void noteLinearAxisTerminalVerificationFailure(
  ...
  if (axis.terminalVerificationFailures >=
    ...
    axis.terminalVerificationReason = ARM_AXIS_VERIFY_NONE;
    SerialDebug.println(
        "[EMM VERIFY] encoder unavailable; retain normal axis timeout");
    return;
  }
  axis.terminalVerificationPending = true;
...
void beginArmTransfer(
  ...
  /*
   * 搬运的M5大角度动作都在M6回安全零点之后启动，可使用仍低于旧版的
   * 中等曲线；端点扫描初始化时会显式切回柔和曲线。
   */
  useArmBaseTransferMotionProfile();
...
bool nominalRingPose(
  ...
  /*
   * 1号按实机最终窗口+30°～+60°，从+45°/90 mm首看；3号保留已经
   * 验证准确的-20°首看。这里只负责首次看见圆，最终点仍由视觉闭环测量。
   */
...
bool predictedRing3SearchPose(ArmPose &pose) {
  ...
  /*
   * 预测结果只保留M6半径；3号M5仍固定为已验证的-20°，失败时只看
   * -30°、-10°。1号搜索窗口的修改不会传递到3号。
   */
  pose.standardFrameAngleDegrees =
      ENDPOINT_RING3_SEARCH_SEED_ANGLE_DEGREES;
...
void beginEndpointScan(uint8_t ringPosition) {
  ...
  if (extensionAxis.active ||
      ...
      fabsf(
          ...
          ARM_AXIS_POSITION_TOLERANCE_MM) {
    routeFault(
      "Initial endpoint scan requires M6 safe zero and M7 raised");
    return;
  }
  ...
  SerialDebug.print(
      "[ENDPOINT SCAN] direct seed ring/M5/M6=");
  ...
  SerialDebug.print(
      ...
      2);
  SerialDebug.print("/");
  SerialDebug.println(searchPose.extensionMm, 2);
}
...
bool beginDirectRing3EndpointScan() {
  if (!liftMoveFinished() ||
      fabsf(
          ...
          M7_STANDARD_HEIGHT_MM) >
          ARM_AXIS_POSITION_TOLERANCE_MM) {
    routeFault("Ring-3 direct handoff requires M7 raised");
    return false;
  }
...
bool startEndpointSearchFallbackMove() {
  ...
  const float fallbackOffsetDirection =
      endpointSearchFallbackMoveCount == 1U
          ...
          : (scanningRing1 ? -1.0f : 1.0f);
  /*
   * 1号候选固定+60°、+30°；3号保持原来的-30°、-10°。
   * 两个端点使用独立窗口，避免再次因修改1号而破坏已经准确的3号。
   */
...
void serviceCompetitionAction() {
  ...
  const bool endpointMapPending =
      endpointMapStartMs != 0UL &&
      endpointMapCompleteMs == 0UL;
  if (endpointMapPending &&
      nowMs - endpointMapStartMs >=
          ENDPOINT_MAP_TOTAL_TIMEOUT_MS) {
    routeFault("Endpoint map total timeout");
    return;
  }
  const bool endpointRingPending =
      endpointMapPending &&
      (activeEndpointScanRing == 1U ||
       activeEndpointScanRing == 3U) &&
      endpointScanStartMs[activeEndpointScanRing] != 0UL &&
      !measuredRingPointValid[activeEndpointScanRing];
  if (endpointRingPending &&
      nowMs -
              endpointScanStartMs[activeEndpointScanRing] >=
          ENDPOINT_RING_SCAN_TIMEOUT_MS) {
    routeFault("Endpoint ring scan timeout");
    return;
  }
  ...
  switch (workActionPhase) {
    ...
    case WORK_PHASE_ENDPOINT_WAIT_COORDINATE: {
      ...
      if (ring == 1U) {
        ...
        SerialDebug.println(
            "[ENDPOINT HANDOFF] ring 1 accepted; raise M7, "
            "then move M5/M6 directly above predicted ring 3");
        workActionPhase =
            WORK_PHASE_ENDPOINT_WAIT_RING3_RAISE;
        break;
      }
