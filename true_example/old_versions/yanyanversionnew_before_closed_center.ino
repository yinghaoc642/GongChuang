...
/*
 * 圆环放料采用两段下降：先按正常速度到最终高度上方20 mm，再用约19%
 * 的速度完成最后20 mm。仅作用于“料盘 -> 圆环”，抓取、抬升、回程和
 * 料盘内部转运仍使用正常速度，兼顾落料稳定性与180秒比赛时限。
 */
constexpr float RING_PLACE_FINAL_DESCENT_MM = 20.0f;
constexpr uint16_t M7_RING_PLACE_SPEED_RPM = 480U;
constexpr uint8_t M7_RING_PLACE_ACCELERATION = 96U;
constexpr uint32_t RING_PLACE_EXTENSION_SETTLE_MS = 100UL;
constexpr uint32_t RING_PLACE_LOWER_SETTLE_MS = 150UL;
...
constexpr float RING_ENDPOINT_MINIMUM_SIDE_SEPARATION_MM = 180.0f;
constexpr float RING_MAP_MAXIMUM_MIDPOINT_ERROR_MM = 60.0f;
constexpr float RING_ENDPOINT_MAXIMUM_AXIS_TILT_DEGREES = 30.0f;
constexpr uint16_t RING_ENDPOINT_MINIMUM_RADIUS_PIXELS = 40U;
constexpr uint16_t RING_ENDPOINT_MAXIMUM_RADIUS_PIXELS = 100U;
...
void startArmTransferDestinationDescent() {
  ...
  SerialDebug.println(
      " mm; final 20 mm will use slow profile");
...
bool buildMeasuredRingMap() {
  ...
  const float endpointAxisAngleDegrees =
      ...
      180.0f / PI_F;
  const float endpointAxisTiltDegrees =
      fabsf(
          fabsf(endpointAxisAngleDegrees) -
          90.0f);
  ...
  SerialDebug.print(
      "[RING MAP] span/error/side/midError/axisTilt/"
      "radiusRatio/scanHeading/mapDrift=");
  SerialDebug.print(endpointSpanMm, 2);
  SerialDebug.print("/");
  SerialDebug.print(spanErrorMm, 2);
  SerialDebug.print("/");
  SerialDebug.print(sideSeparationMm, 2);
  SerialDebug.print("/");
  SerialDebug.print(midpointErrorMm, 2);
  SerialDebug.print("/");
  SerialDebug.print(endpointAxisTiltDegrees, 2);
  SerialDebug.print("/");
...
void serviceCompetitionAction() {
  ...
  switch (workActionPhase) {
    ...
    case WORK_PHASE_ENDPOINT_WAIT_COORDINATE: {
      ...
      SerialDebug.print(
          "[ENDPOINT SCAN] accepted ring/xy/r/conf/"
          "heading/elapsed=");
      SerialDebug.print(ring);
      SerialDebug.print("/");
      SerialDebug.print(x);
