//使用串口舵机V2版库，可自定义通讯串口 https://fashionrobo.com/wp-content//uploads/download/fashionstar-uart-servo-arduino-V2.zip
...
void setup() {
  ...
  digitalWrite(enPin, LOW);    // 使能步进电机 低电平有效
  stepper.setMaxSpeed(60000);  //最高转速1200rpm 每秒20转 *3200脉冲=64000脉冲/秒 最大不要超过60000
...
//阻塞式的会影响数据更新性能
  stepper.runToNewPosition(-6400);  //3200一圈  正负号控制方向检查以下Z向实际运动方向 是上还是下
